...
DoReports(string sChatLine, integer sendNow, integer fromTimer) {
    ...
    if (g_iTraceEnabled) {
        ...
        sLocation += " "+g_sWearerName+" is at "  + sParcelName + " http://maps.secondlife.com/secondlife/"+llEscapeURL(sRegionName)+"/"+(string)llFloor(vPos.x)+"/"+(string)llFloor(vPos.y)+"/"+(string)llFloor(vPos.z);
    }
...
//    llMessageLinked(LINK_SET,NOTIFY,(string)iAlsoNotifyWearer+sMsg,kID);
    ...
    if (kID == g_kWearer) {
        ...
        /////////////////////////////////
        // start LGF Modification
        /////////////////////////////////
        
        //llInstantMessage(kID, sMsg);
        // we request for the online status
        //Debug("Sending request for online status for : " + (string)kID);
        key owner_name_query = llRequestAgentData (kID, DATA_ONLINE);
        g_dataOnlineRequest += (string)owner_name_query;
        g_dataOnlineRequest += kID;
        g_dataOnlineRequest += sMsg;
        /////////////////////////////////
        // start LGF Modification
        /////////////////////////////////
...
*/        
     ...
     dataserver(key queryid, string data){
         ...
         integer n;
         for (n = 0; n < iStop; n += 3) { 
             if (queryid ==  llList2Key(g_dataOnlineRequest,n)) {
                 iKAv = llList2Key(g_dataOnlineRequest,n+1);
                 iMessage = llList2Key(g_dataOnlineRequest,n+2);
                 
                 // we remove the request from the list of request
                 g_dataOnlineRequest = llDeleteSubList(g_dataOnlineRequest, n, n+2);
         ...
         @break;
         //Debug("Dataserver: iKAv=" + (string)iKAv);
         if (iKAv != NULL_KEY) {
             // iKav contains the key on the avatar whose data is provided
             // by dataserver
             //Debug("Dataserver: data=" + (string)data);
             if (data == "1") {
                 llInstantMessage(iKAv, iMessage); 
                 //Debug("Dataserver: envoi d'un message instantané");   
             } else {
                 //user if offline. We send an email
                 
                llMessageLinked(LINK_SET, LM_SEND_EMAIL,(string)iKAv + "|"+ iMessage,NULL_KEY);
                 //Debug("Dataserver: envoi d'un email");   
             }
         }         
     }
